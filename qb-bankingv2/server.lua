...
local function getPlayerPins(citizenid, callback)
    local pins = {}
    
    -- Get PINs from the database
    MySQL.query('SELECT pin FROM bank_pins WHERE citizenid = ?', {citizenid}, function(result)
        if result and #result > 0 then
            for _, pinData in ipairs(result) do
                pins[#pins + 1] = pinData.pin
            end
        end
...
QBCore.Functions.CreateCallback('qb-bankingv2:server:openBank', function(source, cb)
    local src = source
    ...
    getPlayerPins(citizenid, function(pins)

        local bankCards = Player.Functions.GetItemsByName('bank_card')
        if bankCards and #bankCards > 0 then
            for _, bankCard in ipairs(bankCards) do
                if bankCard.info and bankCard.info.cardPin and bankCard.info.cardPin ~= "" then

                    local pinExists = false
                    for _, existingPin in ipairs(pins) do
                        if existingPin == bankCard.info.cardPin then
                            pinExists = true
                            break
                        end
                    end
                    ...
                    if not pinExists then
                        pins[#pins + 1] = bankCard.info.cardPin
                    end
                end
            end
        end
        ...
        cb(accounts, statements, playerData, pins)
    end)
end)

QBCore.Functions.CreateCallback('qb-bankingv2:server:openATM', function(source, cb)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return end
    local bankCards = Player.Functions.GetItemsByName('bank_card')
    ...
    getPlayerPins(citizenid, function(dbPins)
        for _, pin in ipairs(dbPins) do
            local pinExists = false
            for _, existingPin in ipairs(acceptablePins) do
                if existingPin == pin then
                    pinExists = true
                    break
                end
            end
            ...
            if not pinExists then
                acceptablePins[#acceptablePins + 1] = pin
            end
        end
        ...
        cb(accounts, playerData, acceptablePins)
    end)
end)

QBCore.Functions.CreateCallback('qb-bankingv2:server:checkPinSetup', function(source, cb)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return end
    ...
    if not bankCards or #bankCards == 0 then 
        ...
        return cb(false, nil, nil, nil)
    end
    
    local hasPinSetup = false
    local acceptablePins = {}
    ...
    getPlayerPins(citizenid, function(dbPins)
        ...
        cb(true, accounts, playerData, acceptablePins)
    end)
end)

QBCore.Functions.CreateCallback('qb-bankingv2:server:withdraw', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
...
QBCore.Functions.CreateCallback('qb-bankingv2:server:deposit', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    local accountName = data.accountName
...
QBCore.Functions.CreateCallback('qb-bankingv2:server:internalTransfer', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    local job = Player.PlayerData.job
...
QBCore.Functions.CreateCallback('qb-bankingv2:server:externalTransfer', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    local job = Player.PlayerData.job
...
QBCore.Functions.CreateCallback('qb-bankingv2:server:orderCard', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    ...
    exports['qb-inventory']:AddItem(src, 'bank_card', 1, false, info, 'qb-bankingv2:server:orderCard')
    cb({ success = true, message = Lang:t('success.card') })
end)

QBCore.Functions.CreateCallback('qb-bankingv2:server:openAccount', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    local accountName = data.accountName
...
QBCore.Functions.CreateCallback('qb-bankingv2:server:renameAccount', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
...
QBCore.Functions.CreateCallback('qb-bankingv2:server:deleteAccount', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    local accountName = data.accountName
...
QBCore.Functions.CreateCallback('qb-bankingv2:server:addUser', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
...
QBCore.Functions.CreateCallback('qb-bankingv2:server:removeUser', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
...
QBCore.Functions.CreateUseableItem('bank_card', function(source, item)
    ...
    if Player.Functions.GetItemByName(item.name) then
        TriggerClientEvent('qb-bankingv2:client:useCard', source)
    end
end)
...
QBCore.Functions.CreateCallback('qb-bankingv2:server:setATMPin', function(source, cb, data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return cb({ success = false, message = 'Player not found' }) end
    
    local pin = data.pin
    if not pin or pin == '' or string.len(pin) ~= 4 or not tonumber(pin) then
        return cb({ success = false, message = 'Invalid PIN format' })
    end
    
    local bankCards = Player.Functions.GetItemsByName('bank_card')
    if not bankCards or #bankCards == 0 then
        return cb({ success = false, message = 'No bank card found' })
    end
    
    local success = false
    for _, bankCard in ipairs(bankCards) do
        local info = bankCard.info or {}
        info.cardPin = pin
        Player.Functions.RemoveItem('bank_card', 1, bankCard.slot)
        Player.Functions.AddItem('bank_card', 1, bankCard.slot, info)
        success = true
    end
    
    local citizenid = Player.PlayerData.citizenid
    MySQL.insert('INSERT INTO bank_pins (citizenid, pin) VALUES (?, ?)', {
        citizenid,
        pin
    })
    
    if success then
        cb({ success = true, message = 'PIN updated successfully' })
    else
        cb({ success = false, message = 'Failed to update PIN' })
    end
end)
